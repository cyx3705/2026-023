...
本文件适用于整个仓库。**当前源码为 5.4.0，宿主冻结基线为 5.4.0**（DEC-063）。
5.4.0 按用户批准收拢总线前端所有权并收回宿主内部公开面（DEC-063）；v5.3.0、v5.1.2 保留为历史基线。
...
- 当前开发线是 **5.4.x**，源码为 **5.4.0**，冻结基线为 **5.4.0 / `v5.4.0`**。
  本轮经用户明确指定 5.4 并授权跨模块改造（DEC-063）：宿主总线开关封口，前端改由 `IModuleContext.RegisterFrontend` 唯一登记，ServiceHost 与 Services 的内部实现收回 internal，属于明确的版本例外。
  结转前文本保存在 5.4.0 基线的 PreFreeze 文件。冻结后三份 Unshipped 及批准基线仅含 #nullable enable。
