...
本文件适用于整个仓库。**当前源码为 5.3.0，宿主冻结基线为 5.1.2**（DEC-056）。
5.3.0 按用户批准清理过期公开面与实现，具体兼容变化见 DEC-060；不移动冻结标签。
...
- 当前开发线是 **5.3.x**，源码为 **5.3.0**，冻结基线仍为 **5.1.2 / `v5.1.2`**。
  本轮经用户批准删除旧发现、源确认、MCP 专用设置与失效远程确认公开面（DEC-060），属于明确的版本例外。
  三份 Shipped 历史不可改写；删除用 *REMOVED* 进入 Unshipped，新增/改签进入 5.3.0 Unshipped 与批准基线。
