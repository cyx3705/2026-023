...
本文件适用于整个仓库。**当前源码为 5.2.1，宿主冻结基线为 5.1.2**；`HistoryVulcan.Core`
...
- 当前开发线是 **5.2.x**，源码为 **5.2.1**，宿主冻结基线仍为 **5.1.2 / `v5.1.2`**（DEC-056）。
  5.2.1 修按模块卸载漏清待接入条目导致的重复接入（DEC-059），属于冻结合同 §4 第二类
  「正确性缺陷修复」，三份 Shipped 与三份 Unshipped 均一个符号未动。
  5.2.0 是重新冻结前的最后一轮内核清理（DEC-058），公开面净减 10 条、无新增。
