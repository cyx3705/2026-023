...
本文件适用于整个仓库。**当前源码为 5.3.0，宿主冻结基线为 5.3.0**（DEC-061）。
5.3.0 按用户批准完成清理并重新冻结（DEC-060、DEC-061）；v5.1.2 保留为历史标签。
...
- 当前开发线是 **5.3.x**，源码为 **5.3.0**，冻结基线为 **5.3.0 / `v5.3.0`**。
  本轮经用户批准删除旧发现、源确认、MCP 专用设置与失效远程确认公开面（DEC-060），属于明确的版本例外。
  本次批准结转三份 Shipped；结转前文本保存在 5.3.0 基线的 PreFreeze 文件。冻结后三份 Unshipped 及批准基线仅含 #nullable enable。
  后续新增公开面先推进版本；常规删除或改签走主版本并更新消费摘要，不能援引本轮任意删除。
