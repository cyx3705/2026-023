...
# 本脚本在 b-Code-Eng/pipeline（4.6.0 从 Diana/b-Code 迁入；后随 eng 独立出源码树）。
$pipelineRoot = $PSScriptRoot
# pipeline -> b-Code-Eng -> 仓库根 -> HistoryClio 项目库
...
$definitions['HistoryVulcan'] = [ordered]@{
        ...
        BuildScript = 'b-Code-Eng\Build-HistoryVulcanPackage.ps1'
        ...
        GateScripts = @(
            'b-Code-Eng\Test-QualityGate.ps1',
            'b-Code-Eng\Assert-PublicApiBaseline.ps1'
        )
}
