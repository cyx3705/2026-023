...
> 本文抬头曾长期停留在「适用版本 3.3.2」，而正文早已累加到 3.4.x–3.11.x，正式线也已推进到 3.11.3。
...
- 3.11.4 新增候选模块界面试用：`vulcan.module.trialui.load path= alias=` 与
  ...
  `IShellUiRegistrar`，界面不可能在那边创建，也不可能把 WPF 对象递过进程边界；
  住在后台的模块经前端命令代理（前端把整份注册表发布为 `FrontendCapabilityCatalog`，后台因此持有
  `ExecutionSite=Frontend` 代理）调用即中继到前端建界面。候选目录须含 `module.manifest.json`
  且 `ui=true`，校验规则与正式发现同一套；试用界面不进入 `vulcan.module.list`、不注册命令，
  装在独立可卸载 ALC 里，`alias` 是注销粒度且不得与已装载模块同名。
  这是模块作者做界面验收的入口，**不是**新的部署方式。详见 API 手册 §6.4.1。
