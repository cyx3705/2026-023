...
> 本文抬头曾长期停留在旧版本；当前 5.1.2 变化列在本节顶部。
> 版本线推进时必须同步本文抬头，这与同步 `project.manifest.json` 同等重要。

## 主要变化（5.1.2）

- 已通过构建、测试与候选校验的模块安装不再要求第二次确认：`vulcan.dev.submit/finish` 会直接安装并热重载；`vulcan.module.install` 仍只允许本机宿主通道且始终校验 manifest 与 SHA256。
