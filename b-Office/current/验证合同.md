...
## 3.10.1 Release Checks

Verify normal-layout typing stays in the console, popup completion advances through domain, class, method, and parameter after Tab, and the Core public API baseline remains identical to 3.10.0. Version `3.10.1` was formally promoted through Diana after these checks passed.
...
- 控制台冒烟需确认普通停靠与最大化布局都从输入框上方向上弹出候选且输入焦点不丢失；命令、参数名、
  允许值三类候选均可见，`Shift+W` 上移、`Shift+S` 下移，`Tab` 只写入，`Shift+Tab` 不改变候选，`Enter` 执行。
  `Shift+W/S` 循环列表、`Tab` 回填并推进下一层，清空后恢复全部结果。还需验证两种布局共享
  `command.list` 快照、`command.show` 详情缓存和选择状态，
