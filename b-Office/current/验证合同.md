...
| VERIFY-RELEASE | 正式宿主部署前 | Diana 候选审查、人工批准后 `Publish-OneHistoryModule.ps1 -Module HistoryVulcan -Publish`（先部署后提交，见项目概览） | `z-Publish/current` 与正式快照 `z-HistoryVulcan` 文件和 SHA-256 一致 |
...
| 候选产物与测试结果 | `z-Publish/current/` | 否 | 只保留最近一次，可覆盖、可重建 |
...
- 2026-08-16（3.11.6 源码）：MCP 端口固定。占用时失败，不再顺延改写 `mcp.port`。
  `McpGatewayPortTests` 7 通过。
- 2026-08-16（3.11.5 正式发布）：Diana 管线 `20260816-015840-HistoryVulcan`（`publish=true`，主树 `d01216b`）
  ...
  正式宿主进程停止后原子提升 `z-HistoryVulcan`，旧 3.11.4 归档到
...
- 2026-08-16 本地（按模块卸载，REQ-MOD-005 / DEC-042，版本线推进到 3.11.5）：Release 构建 0 error，
  ...
  公开 API 基线 `eng/public-api-baselines/3.11.5/` 通过。
  ...
  候选写入工作区 `z-Publish/current`。未促级正式 z。
...
- 2026-08-16（3.11.4 正式发布）：Diana 管线 `Publish-OneHistoryModule.ps1 -Module HistoryVulcan -Publish`
  ...
  `z-Publish/history/HistoryVulcan/3.11.3-20260815-164759-4608b9f8`，登录启动项已按新正式路径修复。
...
- 2026-08-11（3.10.1 正式发布）：项目合同、3.10.1 公开 API 基线、质量门禁、
  ...
  焦点，Tab 连续推进域、类、方法和参数。候选宿主已生成到 `z-Publish/current`，版本 3.10.1；
...
- 2026-08-11 local (3.5.0 candidate): project contract, `dotnet format --verify-no-changes`,
  ...
  **178/178**. During lock triage, an isolated `artifacts/hv-check1/bin` build/test also passed; the first temp
  ...
  snapshot at `z-Publish/current` with manifest and SHA-256 inventory. No formal Z snapshot or publication was
  performed.
...
- 2026-08-08 本地（3.2.1 命令域与命令类收口）：锁定还原通过；Debug/Release 构建均 0 warning、0 error，
  ...
  `git diff --check` 通过；未生成 `z-Publish/current` 候选，未更新 `z-HistoryVulcan`。
...
- 2026-08-10 本地（3.3.2 九类对齐、模块域去前缀、门禁解耦、旧债退役 / DEC-023）：
  ...
  - 退役：根 `eng/`（2 个无引用脚本）、`Unused/`（27 个模板 CAD 文件）、本地 `artifacts/`（56MB 旧名产物）、
    `Publish-AppShell.ps1`、`z-Publish/history/0.5.0` 与 `0.7.2`、`z-HistoryVulcan/installer.7z` 游离副本、
...
- 2026-08-08 本地（3.2.0 正式发布到 `z-HistoryVulcan`）：正式快照目录由计划的 `z-Package-HistoryVulcan`
  ...
  消费文档同步对齐。旧 3.1.9 候选先整体归档到 `z-Publish/history/3.1.9/`（入库）；脚本重建 3.2.0
...
- 2026-08-07 本地（3.1.5 质量更新）：Debug/Release 构建 0 warning、0 error；外置测试各 142/142；格式、公开 API、项目合同和抑制扫描通过。PackageSmoke 未单独运行，因候选包尚未生成，未触碰 z-Publish 或 z 快照。质量门禁确认抑制标记 0、超过 1000 行的生产热点文件 0。
...
- 2026-08-07 本地（3.1.6 AvalonDock 瞬态异常与宿主部署）：锁定还原通过；Debug/Release 构建均
  ...
  旧 3.1.2 候选已整体归档到 `z-Publish/history/3.1.2`；3.1.6 宿主候选与正式 Z 宿主各 35 个文件，
