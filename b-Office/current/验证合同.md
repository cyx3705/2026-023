...
## 3.11.2 Candidate Checks

Verify focused-console Tab completion advances through domain, class, method, parameter name, registered value, and a following parameter. A positional structural hint that changes neither text nor caret must close without another completion request. Normal-layout input remains plain, and the frozen Core/CommandBus baseline remains unchanged. The formal Z is still 3.11.1.
...
- 专项结论与残余风险见 [命令总线健康检查](命令总线健康检查.md)。
...
- 2026-08-12（3.11.2 参数补全候选）：Release 构建 0 warning/0 error，完整测试 196/196；
  `FocusedConsoleCompletesDomainClassMethodAndParameter` 覆盖参数名 → 注册允许值并在末层关闭，
  ...
  Core、CommandBus、解析器和注册协议均未修改。正式 Z 仍为 3.11.1，未执行正式发布。
