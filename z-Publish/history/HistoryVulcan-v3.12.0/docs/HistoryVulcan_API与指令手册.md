...
§3.3.4 与 `HistoryVulcan_消费变更摘要.md`。当前源码为 **3.12.0**（宿主位于 `z-Publish/host`）。
...
| `vulcan` | `module` | 9 | `vulcan.module.*`（其中 `trialui.*` 2 条只在承载界面的前端注册） |
...
#### 6.4.1 候选模块界面试用（3.11.4）

以下两条按 `ShellConfig.EnableUiModules` 注册，与上表的注册条件不同：它们只出现在**承载界面**
的宿主里。无窗的 `--service` 后台进程没有 `IShellUiRegistrar`，也不可能把 WPF 对象递过进程边界，
因此界面只能长在前端。

| 命令 | 用途 / 关键参数 |
|---|---|
| `vulcan.module.trialui.load path= alias=` | 在前端临时创建候选 z 快照的界面。`path` 须是含 `module.manifest.json` 且 `ui=true` 的目录，清单校验与正式发现同一套规则；`alias` 是界面别名，同时是 `IShellUiRegistrar` 的注销粒度 |
| `vulcan.module.trialui.unload alias=` | 销毁该别名下的界面、注销其 owner 并卸载可卸载程序集 |

试用界面**不进入正式模块快照**：`vulcan.module.list` 与命令表都不因此变化，候选程序集装在独立的
可卸载 ALC 里，卸载即回收；前端退出时残留的试用界面一并拆除。`alias` 不得与已装载模块同名，
否则卸载会连正式模块的工具窗口一起注销——宿主直接拒绝这种别名。

**后台模块怎么用它。** 前端把整份注册表作为 `FrontendCapabilityCatalog` 发布到后台，后台注册表
