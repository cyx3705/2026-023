...
§3.3.4 与 `HistoryVulcan_消费变更摘要.md`。当前源码为 **3.12.1**（宿主位于 `z-Publish/host`）。
...
3.1.9 起模块管理页只读取 `vulcan.module.list`；“刷新模块”执行 `vulcan.module.reload` 后重新取清单；
“热重载”经 `vulcan.ui.selectdirectory` 再执行 `vulcan.module.install`；“打开发现根”执行 `vulcan.module.open`。
模块指令详情统一在命令集页面查看，模块页不再读取 `vulcan.command.list` 或显示第二个指令表。
3.12.1 删除候选试用界面命令；测试与部署都只走 `install`。
